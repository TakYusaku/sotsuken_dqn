...
class DQN:
    ...
    def fitting(self, D, gamma, targetQN):
        ...
        for i, (state_b, action_b, reward_b, next_state_b) in enumerate(mini_batch):
            ...
            # 価値計算
            if not (next_state_b[0] == np.zeros(np.array(state_b[0]).shape)).all(axis=1): # next_state が 0 でない
            ...
            if not (next_state_b[1] == np.zeros(np.array(state_b[1]).shape)).all(axis=1): # next_state が 0 でない
                # 行動決定のQネットワークと価値観数のQネットワークは分離
